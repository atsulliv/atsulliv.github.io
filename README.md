# Welcome to my page :)


## Projects

### Hardware/Systems 💻
- **[From the Ground Up: Blackjack Trainer](https://github.com/atsulliv/107e-blackjacktrainer)**  
  Building bare-metal blackjack card counting game using all of my own, written modules. (No OS or standard library support.)

- **[Circuits I and II: AC-DC Converter + Audio Amplifier](https://github.com/atsulliv/107e-BlackjackTrainer)**

- **[LED Board](https://github.com/atsulliv/LED-Board)**

### Research 🔬
- **[Biosensor FET](https://github.com/atsulliv/IV_processing)**  
  Device-level modeling and analysis of FET-based biosensors.

### In Progress...
- Musical Glove Project
- Musical Arrangment File Management Tool

## Links
- [GitHub](https://github.com/atsulliv)
- [LinkedIn](https://www.linkedin.com/in/aidan-sullivan-25209524a/)
